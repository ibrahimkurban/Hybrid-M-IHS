function [x_out, output] = HyBR_iko(A, b, P, options)
%
...
defaultopt = struct('InSolv','tikhonov','RegPar','wgcv','Omega',...
  ...
  'Vx' , [], 'FlatTol', 10^-6, 'MinTol', 3, 'ResTol', [10^-6, 10^-6]);
...
options = HyBRset_iko(defaultopt, options);
...
adaptWGCV2 = strcmp(regpar, {'wgcv'}) && strcmp(omega, {'corrected'});
...
for i = 1:maxiter+1 %Iteration (i=1) is just an initialization
  [U, B, V] = feval(handle, A, U, B, V, P_le, P_ri, options);
  ...
  if i >= 2 %Begin Lanczos iterations
    ...
    switch insolve
      case {'tsvd', 'tikhonov'}
        ...
        if adaptWGCV2 %Use the adaptive, weighted GCV method
          options.Omega = (i+1)/m;
        end
