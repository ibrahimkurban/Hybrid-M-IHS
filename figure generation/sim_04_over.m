...
setup.hy_iter   = 300;
setup.offset    = 500;
setup.gkl_size  = min(min(n,d), setup.offset+setup.k0);
setup.m         = 2*d;
setup.maxit     = 30;
setup.ptol      = 0;
...
N_met           = 12; %num of methods in the for bloop below
...
for i =1:setup.N_noise
    ...
    %% 3 LS GCV eff
    [x, part, devt]  = LS_gcv_iko(eye(d), setup.sigA,V, Utb(:,i), x1);
    err(k)           = err_x(x,i);
    pare(k)          = part;
    deve(k)          = devt;
    iter(k)          = d;
    ...
    %% 7 PD M-IHS SVD SRHT
    [~, xx, part]    = reg_mihs_gkl_lower_iko(A,b(:,i),m,x1,XTOL,setup.ptol,setup.maxit);
    errs             = err_x(xx,i);
    err(k)           = errs(end);
    pare(k)          = part(end);
    iter(k)          = length(errs);
    ...
    %% 8 PD M-IHS SVD SRHT
    [~, xx, part]    = reg_mihs_gkl_lower_iko(A,b(:,i),2*m,x1,XTOL,setup.ptol,setup.maxit);
    errs             = err_x(xx,i);
    err(k)           = errs(end);
    pare(k)          = part(end);
    iter(k)          = length(errs);
    ...
    %% 9 PD M-IHS SVD SRHT
    [~, xx, part]    = reg_mihs_gkl_lower_iko(A,b(:,i),3*m,x1,XTOL,setup.ptol,setup.maxit);
    errs             = err_x(xx,i);
    err(k)           = errs(end);
    pare(k)          = part(end);
    iter(k)          = length(errs);
    ...
    %% 11 PD M-IHS GKL COUNTSKETCH
    %Count sketch generation
    params              = struct();
    params.SA           = generate_SA_count([A, b(:,i)], 2*m);
    params.Sb           = params.SA(:,end);
    params.SA(:,end)    = [];
    %sovler
    [~, xx, part, out]  = reg_mihs_gkl_lower_iko(A,b(:,i),2*m,x1,XTOL,setup.ptol,setup.maxit,params);
    errs             = err_x(xx,i);
    err(k)           = errs(end);
    pare(k)          = part(end);
    iter(k)          = length(errs);
    ...
    %% 12 PD M-IHS GKL COUNTSKETCH
    %Count sketch generation
    params              = struct();
    params.SA           = generate_SA_count([A, b(:,i)], 3*m);
    params.Sb           = params.SA(:,end);
    params.SA(:,end)    = [];
    %sovler
    [~, xx, part, out]  = reg_mihs_gkl_lower_iko(A,b(:,i),3*m,x1,XTOL,setup.ptol,setup.maxit,params);
    errs             = err_x(xx,i);
    err(k)           = errs(end);
    pare(k)          = part(end);
    iter(k)          = length(errs);
...
save(sprintf('sim_04_over_d%d%d_t%d%d', ttt(3), ttt(2), ttt(4), ttt(5)))
