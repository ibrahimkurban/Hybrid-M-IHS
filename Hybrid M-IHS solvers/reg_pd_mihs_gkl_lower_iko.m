function [x, xx, pars1, pars2, out] = reg_pd_mihs_gkl_lower_iko(A,b,m,x1,xtol,ptol,maxit,params)
...
%
% [x, xx, pars1, pars2, out] = reg_pd_mihs_svd_lower_iko(A,b,m,x1,xtol,ptol,maxit,params)
%
%   params.SAt   = sketch matrix
%   params.WASt  = sketch matrix
%   params.Wb    = sketch mea.
%   params.L     = size of gkl procedure
%   out.iter     = inner iteration numbers
%   out.dev      = if exist dev estimation
%   out.time     = timing
%   
%
% I. Kurban Özaslan
...
while(~EXIT)
    ...
    while(~EXIT2)
        ...
        [par_log,~, ~, outf] = fminbnd(@(lam)fun(10^lam), max(par_log-2, par_low_log), par_log+1, options);  
        in_iter(i,j)        = outf.funcCount;
...
out.L           = L;
out.dev         = dev_est;
out.rp_time1    = rp_time1;
out.rp_time2    = rp_time2;
end
