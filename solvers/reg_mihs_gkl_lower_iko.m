function [x, xx, pars, out] = reg_mihs_gkl_lower_iko(A,b,m,x1,xtol,ptol,maxit,params)
%%REG_MIHS_GKL_LOWER adaptive regularizaiton of m-ihs algorithm
%- gkl is used
%- lower parameter is set by gcv of (SA,Sb)
%
% [x, xx, pars, out] = reg_mihs_gkl_lower(A,b,m,x1,xtol,ptol,maxit,params)
%
...
%% lower bound %!!! L<d -> not resolved, if this is the case discard lower bound from throughout the code 
sig10   = svd(full(R(1:10,1:10)));
if(L < d )
    dev_est         = nan;
    par_low_log     = -14;
    par_log         = log10(sig10(1));
else
    [par_low, dev]  = LS_sgcv_lower_gkl_iko(R,Sb, norm(ASSb));
    %%scale
    par_low_log     = log10(par_low*m/n);
    dev_est         = dev*sqrt(m/n);
    par_log         = log10(sig10(1));%par_low_log+2;
end
